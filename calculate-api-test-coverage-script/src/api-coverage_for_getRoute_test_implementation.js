...
// const ROOT_DIR = '/Users/yash/Desktop/AIC-2019/ai-qa/sso'
...
const ROUTER_ENDPOINT_REGEX = /router\.(get|post|put|delete)\(['"`](\/api\/workspace\/[^'"`]+)['"`]/g;
const APP_ENDPOINT_REGEX = /app\.(get|post|put|delete)\(['"`](\/api\/workspace\/[^'"`]+)['"`]/g;
...
function extractRoutesFromFile(filePath) {
    ...
    while ((match = ROUTER_ENDPOINT_REGEX.exec(fileContents))) {
        routes.push(
            {
                method: match[1],
                endpoint: match[2],
...
function checkTestCoverage(routeObjects) {
    ...
    routeObjects.forEach((route) => {
        ...
        fileContents.forEach((file) => {
            let endpointRegex = new RegExp(endpoint.split('/').at(-1),'g');

            while (endpointRegex.exec(file)) {
                count++;
            }
        });
...
function formatTable(data) {
    ...
    formattedData.sort((a, b) => a.count - b.count);
...
fs.writeFileSync(path.join(__dirname, `../out/api_test_coverage_ai-qa_${ROOT_DIR.split('/').at(-1)}.txt`), formatTable(checkTestCoverage(collectAllEndpoints())));